	...
	if decorated and decorated:type() == "decorated_definition" then
		for i = 0, decorated:child_count() - 1 do
			...
			if
				...
				and vim.treesitter.get_node_text(child, 0):match("fixture")
			then
				return fixture_name
			end
		end
...
local function get_cwd(filepath)
	...
	return vim.fs.root(dir, { "pyproject.toml", "setup.py", "setup.cfg", "pyrefly.toml", ".git" })
		or dir
end
...
local function build_cache_for_file(filepath)
	...
	vim.system(
		...
		{ text = true, cwd = cwd },
		function(result) fixture_cache[filepath] = parse_fixtures_output(result.stdout or "", cwd) end
	)
end
...
local function goto_pytest_fixture(fixture_name, filepath)
	...
	if type(fixture_cache[filepath]) ~= "table" then
		vim.defer_fn(function() goto_pytest_fixture(fixture_name, filepath) end, 300)
		return
	end
...
return {
	on_attach = function(client, bufnr)
		...
		if
			filename:match("/test_[^/]+%.py$")
			or filename:match("/[^/]+_test%.py$")
			or filename:match("/conftest%.py$")
		then
